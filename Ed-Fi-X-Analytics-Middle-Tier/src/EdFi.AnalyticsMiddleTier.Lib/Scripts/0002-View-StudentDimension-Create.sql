...
AS
  ...
  SELECT
    analytics.EntitySchoolYearInstanceSetKey(StudentRaces.StudentUSI, SchoolYear) AS StudentSchoolYearInstanceKey,
    analytics.EntitySchoolYearInstanceSetKey([StudentSchoolAssociation].[SchoolId], SchoolYear) AS SchoolSchoolYearInstanceKey
 ...
 , (SELECT
  ...
  FROM [edfi].[Student]
    ...
    LEFT OUTER JOIN StudentRaces
    ON edfi.Student.StudentUSI = StudentRaces.StudentUSI
    LEFT OUTER JOIN edfi.RaceDescriptor rd
    ON rd.RaceDescriptorId = StudentRaces.RaceDescriptorId
